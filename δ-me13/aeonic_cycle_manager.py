...
from ane_utils import ane_autocast_context
...
class AeonicCycleManager:
    def __init__(self, population_manager: PopulationManager, 
        ...
        logger.info(f"永劫轮回管理器使用计算设备: {self.device}")
    ...
    def aeonic_interaction(self, attacker: Pathstrider, defender: Pathstrider, 
        ...
        if attacker is reincarnator:
            ...
            reward_tensor = torch.tensor([[final_reward]], dtype=torch.float).to(self.device)
            with ane_autocast_context():
                value_loss = self.value_loss_criterion(state_value, reward_tensor)
            value_loss.backward(retain_graph=True)
            self.value_optimizer.step()

            # Action policy network forward + loss
            self.action_policy_network.train()
            with ane_autocast_context():
                action_logits = self.action_policy_network(state)
            action_probs = torch.softmax(action_logits, dim=-1)
