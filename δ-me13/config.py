...
config = {
    # 大模型
    "llm": {
        "gpu_max_count":8, # 最大GPU核心数量 (0表示不启用)
    ...
    # Apple Neural Engine 推理加速
    "ane": {
        "enable_autocast": True,     # 启用 autocast FP16（触达 ANE）
        "llm_mlx_backend": False,   # Windows Server: 用 GGUF / llama.cpp
        "mlx_model_name": "models/Qwen3-0.6B-MLX",
    },
    # 性能调度器
    # 性能调度器（跨平台 NUMA 感知）
    "scheduler": {
        ...
        "alpha": 0.35,                      # EMA 平滑系数
        "interval": 3,                      # 采样间隔（秒）
        "min_threads": 2,                   # 单节点模式最小线程（兼容旧版）
        "min_threads_per_node": 2,          # 每 NUMA 节点最小线程
        "numa_aware": True,                 # 自动检测拓扑并启用 NUMA 感知
        "numa_binding": "strict",           # strict / preferred / none
        "numa_node_threads_override": None, # 手动覆盖，如 [16, 16]
        "apple_silicon_p_only": True,       # macOS：仅用 P 核仿真，E 核留给系统
        "gui_power_enabled": False,         # GUI 功耗监测
    },
}
