...
# 预计算的选票数值矩阵 — 避免每次选举做字符串比较
_RELATIONSHIP_TO_VOTE = {
    "SYNERGY": 0.5,
    "MENTORSHIP": 0.2,
    "REPULSION": -0.7,
    "CLASH": -1.0,
}
VOTE_NUMERIC_MATRIX = np.zeros((len(PATH_NAMES), len(PATH_NAMES)))
for _i in range(len(PATH_NAMES)):
    for _j in range(len(PATH_NAMES)):
        VOTE_NUMERIC_MATRIX[_i, _j] = _RELATIONSHIP_TO_VOTE.get(
            PATH_RELATIONSHIP_MATRIX[_i, _j], 0.0
        )
