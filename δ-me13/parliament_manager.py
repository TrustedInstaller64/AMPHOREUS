...
class ParliamentManager:
    ...
    def hold_election(self, population: list):
        """根据所有实体的投票，分配议会席位（Numba 批量并行）。"""
        if not population:
            return

        from constants import VOTE_NUMERIC_MATRIX
        from human_phase_parallel import _batch_parliament_vote
        all_scores = np.array([(p.score if np.isfinite(p.score) else 0.0) for p in population])
        all_dom_idx = np.array([p.dominant_path_idx for p in population], dtype=np.int64)
        total_votes = _batch_parliament_vote(all_scores, all_dom_idx, VOTE_NUMERIC_MATRIX)
