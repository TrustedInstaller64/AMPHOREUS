...
from human_phase_parallel import (
    _batch_recalculate_and_normalize,
    _batch_parliament_vote, _batch_subsidize_minorities, _batch_conformity_plague,
    extract_entity_arrays, write_back_scores, write_back_affinities,
)
...
# 在 aeonic_cycle_manager.py 和 simulation.py 的开头添加 由AI添加的代码
from device_utils import get_optimal_device, setup_performance_optimizations
from ane_utils import ane_autocast_context, enable_ane_optimizations, is_ane_enabled
from channel import read_command
...
@njit(parallel=True)
def _inorganic_step_numba(activity_arr, stability_arr, grid_size, noise_act, noise_stab):
    """Numba jit 加速的无机阶段单代推演。
    随机噪声从外部传入以保证与 Python 原版相同的随机序列。
    """
    new_activity = np.zeros_like(activity_arr)
    new_stability = np.zeros_like(stability_arr)
            ...
            for di in (-1, 0, 1):
                for dj in (-1, 0, 1):
                    ...
                    if 0 <= ni < grid_size and 0 <= nj < grid_size:
                        ...
                        act_sum += activity_arr[nidx]
                        stab_sum += stability_arr[nidx]
                        count += 1
            avg_act = act_sum / count if count > 0 else 0.0
            avg_stab = stab_sum / count if count > 0 else 0.0
            act_change = (avg_act - activity_arr[idx]) * 0.1 + noise_act[idx]
            stab_change = (avg_stab - stability_arr[idx]) * 0.1 + noise_stab[idx]
            new_activity[idx] = max(0.0, activity_arr[idx] + act_change)
            new_stability[idx] = max(0.0, stability_arr[idx] + stab_change)
    return new_activity, new_stability
class AeonEvolution:
        
# simulation.py 的 __init__ 方法修正版本
    def __init__(self, 
                # --- LLM 触发频率配置 ---
                bard_frequency=10,         
                laertes_frequency=25,        
        ...
        # --- 强化学习与模型 ---
        # 1. 先初始化模型
        hgn_input_size = len(TITAN_NAMES) * 2 + len(PATH_NAMES) * 2 + 1
        ...
        logger.info(f"主模拟使用计算设备: {self.device}")
        
        # 4. 最后将模型移动到设备
        self.guide_network = self.guide_network.to(self.device)
        self.action_policy_network = self.action_policy_network.to(self.device)
        ...
        # --- 管理器实例化 ---
        self.debugger = Debugger(self, gui_mode=gui_mode)
    ...
    def _train_hybrid_guide_network(self, elites, score_reward, diversity_reward):
        ...
        # 修复：显式移动到模型所在设备（此前 state/target 在 CPU，model 在 MPS）
        state = torch.from_numpy(state_np).float().to(self.device)
        target = torch.from_numpy(elite_avg_affinities).float().to(self.device)

        self.guide_network.train()
        self.guide_optimizer.zero_grad()

        with ane_autocast_context():
        ...
        # ⚠️ MPS tensor 不能直接 .numpy()，必须先 .cpu()
        new_base_affinities = predicted_blueprint.detach().cpu().numpy()
    ...
    def _run_one_generation(self):
        ...
        # 交互
        culled_this_gen = set()
        valid_population = list(self.population)
        num_encounters = min(len(self.population), 2000)
        for _ in range(num_encounters):
            ...
            if entity1 not in culled_this_gen and entity2 not in culled_this_gen:
                culled_entity = self.interaction_handler.entity_interaction(
                    entity1, entity2, self.population, self.reincarnator,
                    None, None, 1.0
                )
                if culled_entity:
                    culled_this_gen.add(culled_entity)
                    if culled_entity in valid_population:
                        valid_population.remove(culled_entity)
        ...
        # ── 批量归一化 + 重算（替代 encounter 中的逐次调用）──
        if self.population:
            all_aff, _, _ = extract_entity_arrays(self.population)
            _batch_recalculate_and_normalize(
                all_aff,
                self.population_manager.purity_factor,
                self.population_manager.cosmic_tide_vector,
                self.population_manager.max_affinity_norm,
            )
            write_back_affinities(self.population, all_aff)
    ...
    def _run_inorganic_phase(self, num_generations=50121): 
        ...
        # 转为 numpy 数组供 Numba 使用
        activity_arr = np.array([cell['activity'] for cell in inorganic_pop], dtype=np.float64)
        stability_arr = np.array([cell['stability'] for cell in inorganic_pop], dtype=np.float64)
        n_cells = len(inorganic_pop)
        ...
        for gen in range(num_generations + 1):
            ...
            # 每代生成噪声数组（与原版同 RNG），传入 Numba
            noise_act = np.random.uniform(-0.1, 0.1, size=n_cells)
            noise_stab = np.random.uniform(-0.1, 0.1, size=n_cells)
            activity_arr, stability_arr = _inorganic_step_numba(
                activity_arr, stability_arr, grid_size, noise_act, noise_stab
            )
    ...
    def start(self, num_generations):
        ...
        while self.generation <= TOTAL_SIMULATION_END:
            if self.generation % _heartbeat_mod == 0:
                logger.info("[心跳] Gen %d / %d (%.4f%%)", self.generation, TOTAL_SIMULATION_END,
                            100.0 * self.generation / TOTAL_SIMULATION_END)
            # GUI 模式：管道命令由 debugger 后台线程处理，这里只轮询 paused 状态
            # 非 GUI 模式：键盘监听线程单独处理，不读 stdin（避免 readline 死锁）
            ...
            if self.debugger.paused:
                ...
                if getattr(self.debugger, '_stop_requested', False):
                    logger.info("\n=== 演算已停止（stop 命令）===")
                    return
                continue
            if was_paused:
